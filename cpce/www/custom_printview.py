...
def get_rendered_template(doc, name=None, print_format=None, meta=None,
	...
	if doc.doctype == "Expense Claim":
		attachments = frappe.get_all("File", filters={ "attached_to_doctype": doc.doctype, "attached_to_name": doc.name }, fields=["*"])
		pdf_files = []
		for file in attachments:
			if file.file_name.lower().endswith(".pdf"):
				pdf_files.append(file)
			else:
				#print(frappe.utils.get_files_path(is_private=file.is_private))
